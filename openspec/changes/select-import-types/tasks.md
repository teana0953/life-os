...
- [ ] 更新呼叫端 —— 全 repo 只有兩處 `controller.import(`：`lib/contexts/import/presentation/chaodays_import_screen.dart` 與 `test/contexts/import/presentation/chaodays_import_controller_test.dart` 的 `_import` helper（傳全選以保持既有測試語意）。`chaodays_import_screen_test.dart` 與 `test/app_test.dart` 只建構 controller、簽名不變，預期零改動。
...
- [ ] 匯入中，未選的類型與「已選但還沒輪到」的 leading 目前都是 `circle_outlined`，使用者分不出「這個不會跑」和「這個等一下會跑」。未選的整列降低不透明度來區分（純視覺，不新增狀態）。
- [ ] l10n: **不需要新字串** —— 勾選框以既有的 `importTypeWeight` 等類型名稱為標籤。順手修正 `@importDoneMessage.description` 裡已過時的敘述（它還在講固定跑全部類型）。
