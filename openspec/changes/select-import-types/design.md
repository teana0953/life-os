...
**例外路徑**：
- 全部取消勾選 → 送出按鈕停用。
- 匯入進行中 → 勾選框收起、換成狀態圖示（帳密與日期欄位則是留在原位 disabled —— 它們沒有第二種意義要輪替，勾選框有）。
...
- **匯入中**：勾選框換成狀態圖示（轉圈／成功／失敗）。未被選中的類型整列降低不透明度 —— 否則它與「已選但還沒輪到」都是空心圈，使用者分不出「這個不會跑」和「這個等一下會跑」。
...
- **screen（widget）**：預設全勾；全不勾 → 送出停用；只勾一種 → controller 收到的集合只有那一種；匯入中該列**沒有** `Checkbox`（不是 disabled 的 checkbox）且未選的列較淡；**一輪結束後（成功與失敗兩種）勾選框回來且可改選再送出**（D2 的回歸點）。
