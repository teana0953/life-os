...
- [ ] Two listeners, both **scheduling** rather than checking inline: `_authNotifier` and `_router.routerDelegate`. Schedule via `WidgetsBinding.instance.addPostFrameCallback`, and check `mounted` inside the callback before running (teardown / hot reload would otherwise push onto a disposed router). The delegate listener is what retries after a gate refusal — a gate only returns, it never consumes, so without it a cold start whose post-frame lands before go_router leaves `/splash` would strand the pending until the next foreground (D6). Remove both listeners and dispose the controller in `dispose()`.
