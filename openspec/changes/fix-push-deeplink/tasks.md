...
- [x] A `_gateRefused` flag set whenever `check()` returns at either gate and cleared once a check gets past them, plus `void onNavigation()` which runs `check()` only while that flag is set.
- [x] `start()` registers the lifecycle observer, subscribes to `handoverSignals`, and runs one `check()`; `dispose()` removes the observer and cancels the subscription. Idempotent, mirroring `PwaUpdateController.start()`. Tests drive `check()` / `onNavigation()` directly so they never register a real observer.
