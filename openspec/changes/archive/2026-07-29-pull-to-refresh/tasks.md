...
- [x] `_scheduleLoad()` 由 `void` 改回 `Future<void>`：持 `Completer<void>? _refreshCompleter`，
...
- [x] 新增 `DateTime? _lastOverviewLoadAt`。**看 controller status 判成功，不看 `Future.wait`**：
...
- [x] `HealthScaffold` 已有可注入 `clock`（build 用它算 greeting/day）——確認沿用同一個，
      不要新增第二個 clock 來源

## 3. 總覽 + 趨勢：RefreshIndicator + 時間戳（TDD）

- [x] `_OverviewBody`（health_scaffold.dart:388 的 ListView）包一層
...
- [x] **每個被 RefreshIndicator 包住的 scrollable 都要 `physics: const AlwaysScrollableScrollPhysics()`**
...
- [x] `_TrendBody` 的 ListView 同樣包 `RefreshIndicator`，共用同一個 `onRefresh` callback
      （總覽/趨勢都走 `_load()`）
- [x] `_lastOverviewLoadAt` 透過參數傳進 `_OverviewBody` 與 `_TrendBody`，
      各自頂端放 `LastLoadedLabel`
- [x] widget 測試：兩個分頁各有 `RefreshIndicator`、下拉觸發重載、時間戳顯示注入 clock 的
      時分字串、`null` 時不顯示

## 4. `LastLoadedLabel` 共用 widget（TDD）

- [x] Test first：`test/shared/widgets/last_loaded_label_test.dart` ——
      `null` → `SizedBox.shrink()`（不顯示）；有值 → 顯示「上次更新 <HH:mm>」
- [x] `lib/shared/widgets/last_loaded_label.dart`：吃 `DateTime?`，
      時分用 `TimeOfDay.fromDateTime(dt).format(context)`（跟隨系統 12/24 制），
      弱化色（`textTheme.bodySmall` + `onSurfaceVariant`），純文字（螢幕閱讀器可讀）
- [x] l10n：三個 ARB 檔（app_en.arb 含 `@` 描述、app_zh.arb、app_zh_Hant.arb）
...
- [x] `reloadDay(String day)` 由 `void` 改成 `Future<void>`（tracker_day_nav.dart:21 的
      抽象方法簽章）；`setDay`/`openDefaultCalendar` 內的呼叫不需 await（維持現行行為，
      但可以拿到 future 若之後要用）
- [x] 四個實作（water/vitals/bowel/exercise screen 的 `reloadDay`）改成回 Future ——
...
- [x] **vitals 傳自己的 `onRefresh`**：`hasUnsavedChanges` 為 true 時先跳確認
      （「有未儲存的變更，重新整理會捨棄」走 ARB），確認才 `reloadDay(viewedDay)`，
      否則不重載、手勢直接收；沒有未存變更就照常。**確認閘放在 vitals 畫面，不塞進共用 mixin**
      —— 其餘三畫面沒有 draft-覆寫問題，不該被污染
- [x] 四個畫面在 `dayNavHeader` 之下、內容之上放 `LastLoadedLabel(controller.lastLoadedAt)`
- [x] widget 測試：三個無 draft 畫面下拉會呼叫 `reloadDay(viewedDay)`；
