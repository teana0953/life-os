...
> **已知限制（review 指出，不在本 change 處理）**：`ChaodaysImportController` 是 app 級 singleton（`main.dart`），而 screen 的選擇集合每次進畫面重置成全選。所以離開畫面再回來時，五列會顯示**上一輪**的結果，而那一輪可能是幾天前 —— 畫面上沒有任何時間線索。同一次停留內的一致性沒問題（見「使用者路徑」的說明），跨停留的陳舊則是既有的 singleton 行為在部分重置之後變得更顯眼。要收的話是在進畫面時清掉 `typeStates`，但需要處理「匯入進行中離開再回來」的守門，超出這輪範圍。
...
用 `CheckboxThemeData` 把 disabled 的填色與外框分開指定，拉開兩態對比。

**深色維持現況，但理由要說清楚**：原本寫「深色 3.06:1 已足夠」，那個數字量的是「disabled 元件對卡片的可見度」，**不是兩個 disabled 狀態之間的區隔** —— 深色下未勾與打勾仍是同一個顏色（相距 1.00:1），與淺色修好之後形成不對稱。之所以先不動：淺色是兩者中明顯較差的一邊（1.91:1 對卡片，幾乎看不見），而深色至少元件本身看得清楚、只是兩態相同。列入實機觀察，看得出問題再一起拉開。
...
**補匯單一類型**：跑完一輪後只勾飲食 → 取消掉的那四列在勾選變動的當下就各自清空了（D2）→ 送出 → 只有飲食跑，畫面上只剩飲食的新結果。

**「未選類型保留既有結果」在這個畫面上不可達，是刻意的**：要讓某一型在送出時未被選中，唯一的辦法就是動它的勾選，而動勾選就會清掉它。所以任何有結果的列，必然來自最近一次包含它的那一輪 —— 畫面上不會出現多輪結果並存、需要時間標記來分辨的情況。controller 那條「只重置這輪要跑的」規則仍然要有（它是「不要清掉不相干的東西」這個正確語意的來源），只是 UI 讓它在實務上總是遇到已清空的列。
...
- 動深色主題的 checkbox 對比 —— 見 D4：先記錄已知不對稱並列入實機觀察，不在沒有實機可看的情況下再發明兩個顏色。
