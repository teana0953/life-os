...
| 條件 | 顯示 |
| --- | --- |
| 今天 ∈ **任何一段**已記錄的週期 `[start, end ?? +∞]` | 進行中・第 N 天（預測**還在未來**時才加次要文字「下次預計 {日期}」） |
| 一筆紀錄都沒有 | 還沒有生理期紀錄 |
| 只有一筆（後端要兩筆才算得出週期） | 再記錄一次就能預測下次 |
| `predicted > today` | {日期}・還有 N 天 |
| `predicted == today` | 預計今天 |
| `predicted < today` | 預計 {日期}・已過 N 天還沒有紀錄 |
...
**「進行中」仍然要把預測講出來**（次要文字，例如「下次預計 8月20日」），**但只在它還在未來時**。週期開著沒關滿一個平均週期後，那個預測日會等於今天或已經過去 —— 在 7/28 說「下次預計 7月28日」自相矛盾，標一個過去的日期更是上一段要避免的同一件事。兩條需求在這個交集處相牴觸，裁決是次要那行整個消失：上面的天數已經把事情說完了。很多人只記開始不記結束 —— 那筆一直開著的話，這張卡就永遠不顯示 issue #84 要的那個日期，等於功能對那群人不存在。
...
把一個讀起來像壞掉的日期變成訊號。**措辭要講「紀錄斷了」而不是「月經晚了」**：這個狀態在結構上根本無法區分兩者，而「忘了記」更常見；中文健康語境裡「晚了 N 天」有明確的醫療聯想，而系統手上只有「最後一次起始日 + 平均週期」，那個平均可能只由兩筆紀錄算出。實測輸出包括「已過 120 天」—— 天數愈大，講成醫療事實愈荒謬。**不要把預測夾到未來**（例如一直加週期直到超過今天）：那會把「你已經 26 天沒記錄了」偽裝成一個乾淨的未來日期，是最糟的一種安靜。
