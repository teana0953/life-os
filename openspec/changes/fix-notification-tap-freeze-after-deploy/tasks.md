...
- [ ] 1.1 Record the H2/H3/H4 eliminations as a committed assertion rather than prose: extend
...
- [ ] 2.1 In `lib/shared/pwa/pending_deep_link_web.dart`, make every foreground signal
...
- [ ] 2.2 Give `PendingDeepLinkController` a seam for "the app was brought forward" that is
...
- [ ] 3.1 Bound both loops in `_popTo` (`lib/app.dart`) on "the stack stopped changing" rather
...
- [ ] 3.2 Add the ordinary-case guard: a destination already in the stack under normal
...
- [ ] 4.1 In `web/index.html`, move the `visibilitychange`→visible `checkForUpdate()` off the
...
- [ ] 4.2 Confirm the banner still appears after a deploy: `flutter test test/shared/pwa/pwa_update_banner_test.dart`
...
- [ ] 6.1 `flutter analyze` reports no issues.
- [ ] 6.2 `flutter test` reports `All tests passed!` — read the line, do not infer it from the
  absence of red.
- [ ] 6.3 `npx openspec validate fix-notification-tap-freeze-after-deploy --strict` passes.
