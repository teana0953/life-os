...
#
# actionlint isn't published to npm/pub, so it can't be run via a plain
...
# local directory so repeat runs are fast. Same pattern as
# life-os-backend/scripts/lint-actions.sh.
set -euo pipefail
